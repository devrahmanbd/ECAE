...
def test_memory_store_and_search():
    ...
    result = store_memory(unique_text, metadata, namespace="test")
    assert result is not None
    assert result.text == unique_text

    search_results = search_memory(unique_text, namespace="test")
...
def test_memory_deduplication():
    ...
    # Store first time
    res1 = store_memory(unique_text, namespace="test")
    assert res1 is not None

    # Store second time, should be rejected (return None) due to high similarity
    res2 = store_memory(unique_text, similarity_threshold=0.99, namespace="test")
    assert res2 is None
...
def test_memory_metadata_filtering():
    ...
    store_memory(unique_causal, meta1, namespace="test")

    unique_semantic = f"semantic memory about architecture {uuid.uuid4()}"
    meta2 = MemoryMetadata(memory_type="semantic", tags=["docs"])
    store_memory(unique_semantic, meta2, namespace="test")

    # Search filtering by type
    results_causal = search_memory(unique_causal, memory_type="causal", namespace="test")
    assert all(r.metadata.memory_type == "causal" for r in results_causal if r.metadata)

    # Search filtering by tag
    results_docs = search_memory(unique_semantic, tags=["docs"], namespace="test")
...
def test_memory_relevance_ranking():
    ...
    store_memory(unique_err, namespace="test")
    store_memory(unique_lunch, namespace="test")

    # Should rank the code error higher
    results = search_memory("python index error fix", namespace="test")
